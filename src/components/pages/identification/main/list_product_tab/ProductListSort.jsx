...
const ProductListSort = ({ t, sortProductsBy }) => (
  <Menu as={"menu"} className="toolbar-sort md:hidden relative inline-block">
    ...
    >
      <Menu.Items className={"absolute -right-2 mt-2 w-max px-2 py-1 origin-top-right grid grid-flow-row gap-1 items-center rounded-lg bg-green-50 dark:bg-gray-900 shadow-md dark:shadow-white/50 font-sans text-sm text-center"}>
        {en.sort_products.slice(2).map((sortBy, i) => (
