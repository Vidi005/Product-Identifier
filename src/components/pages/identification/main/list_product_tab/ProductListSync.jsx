...
const ProductListSync = ({ t, onClickSyncBtn, isSyncBtnClicked }) => isSyncBtnClicked
  ? (
    <button className="toolbar-sync md:flex items-center bg-orange-700 mr-2 md:ml-1 md:mr-0 p-1 md:p-2 rounded-lg" disabled>
  ...
  : (
    <button className="toolbar-sync md:flex items-center bg-green-50 md:bg-green-700 dark:bg-gray-300 dark:md:bg-green-800 md:hover:bg-green-500 dark:md:hover:bg-green-600 active:bg-green-400 md:active:bg-green-500 dark:active:bg-white md:dark:active:bg-green-600 mr-2 md:ml-1 md:mr-0 p-1 md:p-2 duration-200 rounded-lg shadow dark:shadow-white" onClick={() => onClickSyncBtn()}>
      ...
      <p className="hidden md:block text-white text-sm">{t('synchronize_data')}</p>
    </button>
    )

export default withTranslation()(ProductListSync)