...
const HeaderContainer = ({ props, isUserLoggedIn, isUserActivated, headerTitle, changeLanguage, setDisplayMode, isDarkModeEnabled }) => {
  const { t } = props
  return (
    <header className="identifier-app__header relative flex flex-nowrap items-center justify-between max-w-full bg-green-700 p-1 shadow-xl">
      <section className="grow flex items-center p-1">
        <img className="hidden md:block max-h-6 object-contain object-center mx-2" src="images/pi-logo-dark.svg" alt="Logo" />
        <h3 className="font-serif text-white">{headerTitle}</h3>
      </section>
      <section className="w-fit flex items-center pl-1">
        ...
        <Menu as={"menu"} className="inline-block h-10 pl-2">
          ...
          >
            <Menu.Items className={"absolute right-1 md:right-16 mt-2 w-40 p-1 origin-top-right grid grid-flow-row gap-1 items-center rounded-lg bg-green-700 shadow-lg text-base z-20"}>
        ...
        <Menu as={"menu"} className="inline-block h-10">
          <Menu.Button className="inline-flex w-full items-center justify-center h-full p-2 hover:bg-green-900 focus-visible:ring-2 focus-visible:ring-white/75 duration-200 rounded-md">
            <img className="h-full object-contain" src="images/account-icon.svg" alt="User" />
            <img className="h-full object-contain" src="images/expand-icon.svg" alt="Expand" />
          </Menu.Button>
          <Transition
            as={Fragment}
          ...
          >
            <Menu.Items className="absolute right-1 mt-2 w-40 p-1 origin-top-right grid grid-flow-row gap-1 items-center rounded-lg bg-green-700 shadow-lg text-base z-20">
              <Menu.Item as={Link} to="/account/settings" className="flex items-center text-white hover:bg-green-200 hover:text-green-900 cursor-pointer rounded-md p-2 duration-200 animate__animated animate__fadeInRight animate__faster">
                <img className="mr-2" src="images/settings-icon.svg" alt="Setting" />
                {t("account.0")}
              </Menu.Item>
              <Menu.Item as={"span"} className="flex items-center text-white hover:bg-green-200 hover:text-green-900 cursor-pointer rounded-md p-2 duration-200 animate__animated animate__fadeInRight animate__faster">
                <img className="mr-2" src="images/logout-icon.svg" alt="Logout" />
                {t("account.1")}
              </Menu.Item>
            </Menu.Items>
          </Transition>
